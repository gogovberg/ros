...
void baseStatusCallback(const actionlib_msgs::GoalStatusArray::ConstPtr &msg)
{
    ...
    for (int i=0; i<msg->status_list.size(); i++)
    {
        ...
        if (msg->status_list[i].status == msg->status_list[i].PENDING || msg->status_list[i].status == msg->status_list[i].ACTIVE || msg->status_list[i].status == msg->status_list[i].RECALLING || msg->status_list[i].status == msg->status_list[i].PREEMPTING)
        {
            hasActiveGoal = true;
            
            // DEBUG_PRINT("hasActiveGoal == true");
        }
    }
...
void initGoals()
{
	...
	goal1.pose.position.x = 0.500000;
	goal1.pose.position.y = 0.450000;
	
	geometry_msgs::PoseStamped goal2;
	goal2.header.frame_id = "map";
	goal2.pose.orientation.w = 1;
	goal2.pose.position.x =-2.100000;
